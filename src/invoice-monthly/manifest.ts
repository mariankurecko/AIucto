...
export function buildManifest(params: {
  ...
  const documents: ManifestDocumentRecord[] = params.documents.map((document) => ({
    ...
    invoiceDate: document.invoiceDate ?? document.document?.issueDate ?? document.issueDate ?? null,
    deliveryDate: document.deliveryDate ?? document.document?.taxableSupplyDate ?? document.taxableSupplyDate ?? null,
    detectedPeriod: document.detectedPeriod ?? (document.invoiceDate ?? document.document?.issueDate ?? document.issueDate ?? document.deliveryDate ?? document.document?.taxableSupplyDate ?? document.taxableSupplyDate ?? null)?.slice(0, 7) ?? null,
    ...
    driveUrl: document.driveUrl ?? document.driveFileUrl ?? document.reviewDriveFileUrl ?? null,
    driveFileUrl: document.driveFileUrl ?? document.reviewDriveFileUrl ?? null,
    uploadError: document.uploadError ?? null,
