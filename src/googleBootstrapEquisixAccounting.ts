...
const resolvedToken = loadGoogleTokenOrThrow("equisix-google-primary", "drive_sheets");
const tokenRecord = resolvedToken.record;
...
if (
  ...
  tokenRecord.connection_id !== "equisix-google-primary"
) {
  fail("Google invoice-core token metadata does not match Equisix.");
}

if (
  !Array.isArray(tokenRecord.scopes) ||
  !tokenRecord.scopes.includes("https://www.googleapis.com/auth/drive.file")
) {
...
console.log(`Resolved token: ${resolvedToken.filename}`);
console.log("");
...
async function ensureFolder(
  ...
  const response = await drive.files.create({
    requestBody: {
      name,
      mimeType: FOLDER_MIME as string,
      parents: [parentId],
      appProperties: {
        marianAiOs: "true",
        accountId,
